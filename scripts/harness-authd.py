...
COMMANDS={
 ...
 ('claude','account'): ['claude'], ('opencode','connect'): ['opencode'], ('opencode','interactive'): ['opencode'], ('github','device'): ['gh','auth','login','--web','--hostname','github.com','--git-protocol','https','--skip-ssh-key'],
    ...
    while True:
        ...
        with LOCK:
            ...
            for u in URL_RE.findall(clean):
                if s.get('harness')=='opencode' and 'auth.openai.com/codex/device' in u:
                    continue
                ru=redact(u); 
