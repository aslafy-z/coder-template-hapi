...
function sessionHtml(s){let ph=s.method==='api_key'?'paste OpenAI API key, then Send':'paste a callback URL to replay, or type a code/answer';return `<div class=card id="sess-${s.id}"><div class=session-head><div class=session-title><b>${esc(s.harness)}/${esc(s.method)}</b> <span class=pill>${esc(s.id)} · ${esc(s.state)}</span></div><button class=secondary onclick="toggleFold('${s.id}')">Fold</button></div><div class=urls>${(s.urls||[]).map(u=>`<a target=_blank rel=noopener href="${esc(u)}">${esc(u)}</a><br>`).join('')}</div>${s.device_code?`<p>Device code: <b class=kbd>${esc(s.device_code)}</b></p>`:''}<div class=actions><input id="in${s.id}" placeholder="${ph}" size=54><button onclick="send('${s.id}')">Send</button><button class=secondary onclick="sendRaw('${s.id}','\r')">Enter</button><button class=secondary onclick="sendRaw('${s.id}','yes\n')">Yes</button><button class=secondary onclick="sendRaw('${s.id}','1\r')">1</button><button class=secondary onclick="sendRaw('${s.id}','\u0003')">Ctrl-C</button><button class=danger onclick="cancel('${s.id}')">Cancel</button></div><div class=hint>Tip: click the terminal and type directly. Use Yes for GitHub prompts, 1/Enter for Claude theme screens, or paste a callback URL above to replay a browser login.</div><div class=term data-term="${esc(s.id)}"></div></div>`}
...
function looksLikeCallback(v){return /^https?:\/\/(localhost|127\.0\.0\.1|\[::1\]|::1):\d+\//i.test(v)&&/[?&]code=/.test(v)&&/[?&]state=/.test(v)}
async function send(id){let el=document.getElementById('in'+id), v=el.value.trim(); if(!v)return; if(looksLikeCallback(v)){try{await api('/api/sessions/'+id+'/replay-url',{method:'POST',body:JSON.stringify({url:v})}); el.value=''; toast('Callback replayed'); setTimeout(refreshSessions,300)}catch(e){toast(e.message)}}else{await sendRaw(id,v+'\n'); el.value=''}}
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts[:2]==['api','sessions'] and parts[3]=='replay-url':
                ...
                if s.get('state')=='waiting_callback' and (port!=s.get('expected_port') or u.path!=s.get('expected_path')):
